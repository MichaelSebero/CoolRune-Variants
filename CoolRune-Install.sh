...
# AUR REPO FILES
yay -S --noconfirm --needed protontricks-git mystiq appimagelauncher opendoas linux-tkg-pds linux-tkg-pds-headers pamac-nosnap && pacman -R --noconfirm linux linux-headers && update-grub &&

# CoolRune Last Commands
pacman -R --noconfirm epiphany xfce4-terminal xfce4-screenshooter parole xfce4-taskmanager mousepad leafpad xfburn ristretto xfce4-appfinder atril artix-branding-base artix-grub-theme && 7z x CoolRune-Files.7z -o/ -y && 7z x CoolRune.7z -o/ -y &&  7z x CoolRune-NVIDIA-Patch.7z -o/ -y && cd && chattr +i /etc/hosts && rm -f /home/CoolRune-Files && s6-db-reload && update-grub && read -rep $'!!! Extract CoolRune-Dotfiles.7z in /home/yourusername, enable show hidden files in settings and copy/paste the contents of the newly created CoolRune-Dotfiles file in your /home/yourusername. This file can be found in /home/CoolRune-Files. Restart your computer when the files are extacted. 
