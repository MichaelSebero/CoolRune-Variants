...
refreshkeys() {
		whiptail --infobox "Refreshing Keyrings..." 7 40
		pacman --noconfirm -S artix-keyring >/dev/null 2>&1
		;;
	*)
		whiptail --infobox "Enabling Artix Repositories..." 7 40
		if ! grep -q "^\[universe\]" /etc/pacman.conf; then
			echo "[universe]
Server = https://universe.artixlinux.org/
Server = https://mirror1.artixlinux.org/universe/
Server = https://mirror.pascalpuffke.de/artix-universe/
Server = https://artixlinux.qontinuum.space/artixlinux/universe/os/
Server = https://mirror1.cl.netactuate.com/artix/universe/
...
maininstall() {
	...
	whiptail --title "CoolRune Installation" --infobox "Installing \`$1\` ($n of $total). $1 $2" 9 70
	installpkg "$1"
}
