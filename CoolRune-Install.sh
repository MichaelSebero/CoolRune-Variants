...
welcomemsg() {
	whiptail --title "Welcome!" \
		--msgbox "Welcome to Luke's Auto-Rice Bootstrapping Script!(Sols' editon)\\n\\nThis script will automatically install a fully-featured Linux desktop, which I use as my main machine.\\n\\n-Luke" 10 60
...
refreshkeys() {
	...
	*systemd*)
		...
		;;
	*)
		whiptail --infobox "Enabling Arch Repositories and Choatic-AUR..." 7 40
...
Server = https://ftp.crifo.org/artix-universe/" >>/etc/pacman.conf
		...
		if ! grep -q "^\[choatic-aur\]" /etc/pacman.conf; then
			echo "[chaotic-aur]
