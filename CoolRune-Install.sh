...
pacman -R --noconfirm linux linux-headers epiphany xfce4-terminal xfce4-screenshooter parole xfce4-taskmanager mousepad leafpad xfburn ristretto xfce4-appfinder atril artix-branding-base artix-grub-theme mpv xfce4-sensors-plugin xfce4-notes-plugin && 7z x CoolRune-Files.7z -o/ -y && 7z x CoolRune.7z -o/ -y && 7z x CoolRune-NVIDIA-Patch.7z -o/ -y  &&chattr +i /etc/hosts && s6-service add default apparmors6 && s6-db-reload && grub-mkconfig -o /boot/grub/grub.cfg && mount /boot/efi && grub-install && update-grub && read -rep $'!!! Extract CoolRune-Dotfiles.7z in /home/yourusername, enable show hidden files in settings and copy/paste the contents of the newly created CoolRune-Dotfiles file in your /home/yourusername. This file can be found in /home/CoolRune-Files. Press enter to restart your computer when the files are extacted. ' && reboot
